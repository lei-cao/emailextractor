$('table').css('background-color', '#e2e2e2');
$("a:contains('Detailed')").css({'color':'green', 'weight': 'bold'});
$('body').prepend('<div style="background:#9fff32;" id="clickAllLinks"><a style="font-size: 16px; line-height:16px; font-weight: bold; color: red;" href="#">Click Me To Got All Emails</a></div>');
var articleLinks = [];
var articleLink = '';
var active = true;
...
self.port.emit("journalTitle", ['ei',document.location.toString()]);

$('a:contains("Detailed")').live('click',function(event) {
	$(this).css({'color':'red'});	
	articleLink = 'http://www.engineeringvillage2.org' + $(this).attr('href');
	articleLinks.push(articleLink);
...
function anchorClicker() {
	$('a:contains("Detailed")').each( function() {
		var $anchor = $(this);
