...
$('#SubjectManifest > li').each( function() {
	var $this = $(this);
	var $liHtmlAnchor = $(this).find('li.html a');	
	if ($liHtmlAnchor.length) {
		$liHtmlAnchor.css({'color':'green'}).live('click',function(event) {
			/* article Title */
			var articleTitle = $(this).parent().parent().parent().find('p.title').text();
			console.log('articleTitle: ' + articleTitle);
			self.port.emit("articleTitle", articleTitle);
			
			/* article Link */
			$(this).css({'color':'red'});	
			articleLink = 'http://www.springerlink.com' + $(this).attr('href');
			articleLinks.push(articleLink);
...
function anchorClicker() {
	$('#SubjectManifest > li').each( function() {
		var $anchor = $(this).find('li.html a');
		if ($anchor.length) {
